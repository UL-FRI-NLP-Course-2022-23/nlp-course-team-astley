        ...
        with open('ROC_dataset/test.csv', 'r') as csvfile:
            ...
            for i, row in enumerate(reader):
                ...
                for i, character in enumerate(characters):
                    if base_sentiment[i][character] is 'None':
                        print("No sentiment for character:", character)
                        story_results.append((0, perp_score))
                    else:
                        print("Character:", character)
                        char_sent = (character, base_sentiment[i][character])
                        for index in outputs["endings"]:
                            sentiment = sentiments[index]
                            if sentiment[character] is 'None':
                                print("No sentiment in ending:", index)
                                story_results()
                            else:
                                print("SENTIMENT for ending", index, "is", sentiment)
                                if sentiment[character] is negated_character_sentiments[character]:
                                    print("Sentiment kept")
                                else:
...
"""
        ...
        # check sentiment per character
        for character in characters:
            if len(base_sentiment[character]) == 0:
                print("No sentiment for character:", character)
            else:
                print("Character:", character)
                for index in outputs["endings"]:
                    sentiment = sentiments[index]
                    if len(sentiment[character]) == 0:
                        print("No sentiment in ending:", index)
                    else:
                        print("SENTIMENT for ending", index, "is", sentiment)
                        print("")
